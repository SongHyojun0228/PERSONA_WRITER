...
if (!supabaseUrl || !supabaseKey || !supabaseServiceKey) {
    throw new Error("Supabase URL, Anon Key, and Service Key are required in .env file");
}
...
app.get("/api/published-stories", async (req, res) => {
    ...
    try {
        ...
        `);
            ...
            if (error)
                throw error;
            finalStoriesQuery = data.sort((a, b) => (b.likes[0]?.count || 0) - (a.likes[0]?.count || 0));
        }
        else {
            // Default to 'newest'
            let queryBuilder = supabase.from("published_stories").select(`
        ...
        const storiesWithUsernames = await Promise.all(finalStoriesQuery.map(async (story) => {
            const { data: { user }, error: userError, } = await supabaseAdmin.auth.admin.getUserById(story.user_id);
        ...
        try {
            ...
            if (subscribers && subscribers.length > 0) {
                ...
                if (notificationError) {
                    ...
                    console.log(`Created ${notificationsToInsert.length} notifications for new story.`);
                }
            }
        }
        catch (e) {
...
app.post("/api/generate-image", async (req, res) => {
    ...
    const IMAGE_PROMPT_PREFIX = "Generate a highly detailed, photorealistic image of the following scene: ";
    try {
        let result;
        try {
            ...
            console.error("Error directly from imageGenModel.generateContent:", apiError);
        ...
        let imageUrl; // Declare once here
        if (inlineData?.data && inlineData?.mimeType) {
            ...
            res.status(200).json({
                ...
                message: "Gemini model returned a text message instead of an image. Please refine your prompt.",
            });
        }
        else {
            throw new Error("No valid image data or text response found in Gemini response.");
        }
    }
    catch (error) {
...
app.get("/api/users/:userId/profile", async (req, res) => {
    ...
    try {
        ...
        console.error(`Unexpected error fetching profile for user ${userId}:`, error.message);
...
app.get("/api/users/:userId/stories", async (req, res) => {
    ...
    try {
        ...
        const { data: { user: author }, error: authorError, } = await supabaseAdmin.auth.admin.getUserById(userId);
        if (authorError) {
...
app.get("/api/users/:userId/subscription-status", getUserFromToken, async (req, res) => {
...
app.delete("/api/users/:userId/subscribe", getUserFromToken, async (req, res) => {
    const subscribedToId = req.params.userId;
    const currentUser = req.user;
    if (!currentUser) {
        return res
...
app.post("/api/projects/:projectId/relationships", getUserFromToken, async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        const newRelationship = { ...data, source_name: data.source?.name, target_name: data.target?.name };
...
app.post("/api/notifications/mark-all-as-read", getUserFromToken, async (req, res) => {
    const currentUser = req.user;
    if (!currentUser) {
        return res.status(401).json({ error: "You must be logged in." });
    }
...
app.get("/api/published-stories/:id/comments", async (req, res) => {
    ...
    try {
        const { data: comments, error } = await supabase
            .from("comments")
            .select("id, content, created_at, user_id, parent_comment_id, is_anonymous") // Include new fields
            .eq("published_story_id", published_story_id)
        ...
        console.error(`Error fetching comments for story ${published_story_id}:`, error);
...
app.post("/api/published-stories/:id/comments", async (req, res) => {
    ...
    if (is_anonymous) {
        ...
        const { data: { user }, error: userAuthError, } = await supabaseUserClient.auth.getUser();
        if (userAuthError || !user) {
            console.error("Error getting user for non-anonymous post:", userAuthError);
            return res
                .status(401)
...
app.get("/api/search", async (req, res) => {
    ...
    try {
        ...
        const storySearchPromise = supabase
            ...
            return Promise.all((storyData || []).map(async (story) => {
                const { data: { user }, error: userError, } = await supabaseAdmin.auth.admin.getUserById(story.user_id);
                return {
                    ...story,
                    profiles: {
    ...
    try {
        if (lang === "ko") {
            const result = await new Promise((resolve, reject) => {
                ...
                spellCheckByDAUM(text, 10000, (partialResults) => {
            ...
            return res
                .status(400)
                .send("Unsupported language for spell checking. Only 'ko' and 'en' are supported.");
        }
    }
    catch (error) {
...
app.post("/api/check-consistency", async (req, res) => {
    ...
    if (!storyText || !characterSheet) {
        return res
            .status(400)
            .send("Story text and character sheet are required for consistency check.");
    }
    const systemInstruction = {
        role: "system",
        parts: [
...
app.post("/api/chat", async (req, res) => {
    try {
        const { message, history, context, username, // Add username to the request body destructuring
...
app.listen(port, () => {
    console.log(`[Persona Writer] Backend server is running at http://localhost:${port}`);
});
